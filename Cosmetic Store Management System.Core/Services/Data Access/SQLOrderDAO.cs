...
public class SQLOrderDAO : IOrderDAO
{
    ...
    //public Tuple<List<Order>, int> GetOrders(int page, int rowsPerPage, DateTime? startDate = null, DateTime? endDate = null)
    //{
    //    if (page < 1) page = 1;
    //    List<Order> orders = new List<Order>();
    //    NpgsqlConnection connection = DBConnection.GetConnection();
    ...
    //    // Adjusted query to include optional date range filtering
    //    command.CommandText = $"""
    //    SELECT count(*) over() as TotalOrders, o.order_id, o.customer_id, o.subtotal, o.discount, o.sale_tax, o.total, o.order_date, cu.customer_name, cu.phone
    //    FROM "ORDERS" o
    //    LEFT JOIN "CUSTOMER" cu on o.customer_id = cu.customer_id
    //    {((startDate.HasValue && endDate.HasValue) ? "WHERE o.order_date BETWEEN @StartDate AND @EndDate" : "")}
    //    {(rowsPerPage > 0 ? "OFFSET @Skip LIMIT @Take" : "")};
    //""";
